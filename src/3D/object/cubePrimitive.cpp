...
void CubePrimitive::drawWireframeOverride()
{
...
void CubePrimitive::drawBoundingBoxOverride() {
	ofVec3f scale = getMeshBoundingBoxDimension(cube->getMeshPtr());
	//float scaleFactor = model->getNormalizedScale();
	float scaleFactor = -1;
	scale = (scale * scaleFactor) / 2;
	ofVec3f pos1 = cube->getPosition();
...
ofVec3f CubePrimitive::getMeshBoundingBoxDimension(ofMesh* mesh) {
